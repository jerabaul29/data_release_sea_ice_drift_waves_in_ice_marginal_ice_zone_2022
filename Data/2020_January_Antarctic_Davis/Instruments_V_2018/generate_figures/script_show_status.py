...
from params import list_instruments, time_start, time_end
...
for crrt_key in keys_sorted_by_date:
    ...
    if crrt_time > time_start and crrt_time < time_end:
        ...
        print("***** WARNING time out of bounds: {}".format(crrt_time))
...
if list_instruments is None:
    list_instruments = dict_data_each_logger.keys()
print("using instruments for show_status: {}".format(list_instruments))

# a few sanity checks
for crrt_logger_ID in list_instruments:
...
for crrt_ind, crrt_logger_ID in enumerate(list_instruments):
