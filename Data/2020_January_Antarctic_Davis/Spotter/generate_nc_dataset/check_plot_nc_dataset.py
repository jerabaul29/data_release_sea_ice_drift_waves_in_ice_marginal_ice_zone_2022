...
input_file = "./data_Spotter_Antarctic_Davis_2020_01.nc"
...
with nc4.Dataset(input_file, "r", format="NETCDF4") as nc4_fh:
    ...
    for ind_trajectory in range(number_of_trajectories):
        ...
        array_tz0 = nc4_fh["tm"][ind_trajectory, :]
        crrt_dict_wave_stats = {}
        ...
        for crrt_index, crrt_kind in enumerate(array_kind):
            ...
            crrt_dict_wave = {}
            crrt_dict_wave_stats[crrt_datetime] = {}
            crrt_dict_wave_stats[crrt_datetime]["hs"] = float(array_hs[crrt_index])
            crrt_dict_wave_stats[crrt_datetime]["tp"] = float(array_tp[crrt_index])
            crrt_dict_wave_stats[crrt_datetime]["tm"] = float(array_tz0[crrt_index])

            if crrt_kind == "B":
                crrt_dict_wave["spectrum"] = array_spectrum[crrt_index, :].data
                dict_time_wave[crrt_datetime] = crrt_dict_wave
            elif crrt_kind == "N" or crrt_kind == "S":
                pass
            else:
        ...
        dict_extracted_data[crrt_instrument_id]["wave_stats"] = crrt_dict_wave_stats
...
for crrt_index, crrt_logger_ID in enumerate(list_keys_ordered_by_first_lat):
    ...
    crrt_spectra_logged = np.log10(np.array(crrt_spectra))
    ...
    pclr = plt.pcolor(crrt_datetimes_holed[1:], frequencies, np.transpose(crrt_spectra_logged_holed)[:, 1:], vmin=-4.0, vmax=-1.0)
...
for crrt_index, crrt_key in enumerate(list_keys_ordered_by_first_lat):
    ...
    crrt_dict_wave_stats = dict_extracted_data[crrt_key]["wave_stats"]
    crrt_list_datetimes_time_waves = sorted(list(crrt_dict_time_waves.keys()))
    crrt_list_datetimes_wave_stats = sorted(list(crrt_dict_wave_stats.keys()))
    compute_wave_spectrum_moments_stats(crrt_dict_time_waves)

    crrt_list_Hs = [crrt_dict_wave_stats[crrt_datetime]["hs"] for crrt_datetime in crrt_list_datetimes_wave_stats]
    crrt_list_Hs_proc = [crrt_dict_time_waves[crrt_datetime]["Hs_proc"] for crrt_datetime in crrt_list_datetimes_time_waves]
    max_SWH = max(max_SWH, max(crrt_list_Hs))

    crrt_list_Tp = [crrt_dict_wave_stats[crrt_datetime]["tp"] for crrt_datetime in crrt_list_datetimes_wave_stats]
    crrt_list_Tm = [crrt_dict_wave_stats[crrt_datetime]["tm"] for crrt_datetime in crrt_list_datetimes_wave_stats]
    ...
    if crrt_index == len(list_keys_ordered_by_first_lat)-1:
        ax1.plot(crrt_list_datetimes_wave_stats, crrt_list_Hs, color=crrt_color, linestyle="-.", marker="*", label="Hs")
        ax1.plot(crrt_list_datetimes_time_waves, crrt_list_Hs_proc, color=crrt_color, linestyle=":", marker="o", label="Hs_proc")

        ax1.axis(xmin=time_start_md, xmax=time_end_md)
        ax1.axis(ymin=0.01, ymax=max_SWH+0.2)
        ax1.set_xticks([])
        ax1.legend(loc="upper left", ncol=3, fontsize=12)
        ax1.set_ylabel("SWH (m)")
    else:
        ax1.plot(crrt_list_datetimes_wave_stats, crrt_list_Hs, color=crrt_color, linestyle="-.", marker="*")
        ax1.plot(crrt_list_datetimes_time_waves, crrt_list_Hs_proc, color=crrt_color, linestyle=":", marker="o")

    # plot on the bottom subplot: wave period properties
    # is it time to set the axis, legend, etc, properties?
    if crrt_index == len(list_keys_ordered_by_first_lat)-1:
        ax2.plot(crrt_list_datetimes_wave_stats, crrt_list_Tp, color=crrt_color, linestyle="--", marker="+", label="Tp {}".format(crrt_key))
        ax2.plot(crrt_list_datetimes_wave_stats, crrt_list_Tm, color=crrt_color, linestyle="-.", marker="*", label="Tz0")
        ...
        ax2.plot(crrt_list_datetimes_wave_stats, crrt_list_Tp, color=crrt_color, linestyle="--", marker="+", label="Tp {}".format(crrt_key))
        ax2.plot(crrt_list_datetimes_wave_stats, crrt_list_Tm, color=crrt_color, linestyle="-.", marker="*")
