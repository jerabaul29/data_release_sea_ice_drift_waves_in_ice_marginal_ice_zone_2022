...
from hex_rawstring_decoder import GNSS_Packet as GNSS_Packet_hex
...
list_instruments_use_spectra = ["19633",
                                "19643",
                                "19648",
                                "19636",
                                "19612",
...
date_end_ice_spectra = datetime.datetime(2021, 3, 18, 18, 0, 0)
...
list_frequencies = all_data["19633"]["spectra"][0].list_frequencies
nbr_of_frequency_bins = len(list_frequencies)

# ------------------------------------------------------------------------------------------
print("***** prepare the dataset")

output_file = "data_drift_waves_Barents_2021_02_proto.nc"
...
with nc4.Dataset(output_file, "w", format="NETCDF4") as nc4_out:
    ...
    nc4_out.title = "Sea ice drift and wave measurements from the Nansen Legacy Cruise PC-2: Winter Process Cruise (WPC), 2021, using prototype instruments"

    nc4_out.summary = \
        "The sea ice drifter trajectories and wave measurements from the Nansen Legacy Cruise PC-2: Winter Process Cruise 2021. The " +\
        "trajectories and wave measurements were obtained by prototype instruments deployed on the ice. For more information " +\
        ...
        "https://github.com/jerabaul29/data_release_sea_ice_drift_waves_in_ice_marginal_ice_zone_2022/tree/master/Data/2021_February_Barents/Instruments_V_2021a/generate_nc_dataset . " +\
    ...
    nc4_out.date_created = "2021-09-13:00:00:Z"
    ...
    max_datetime = datetime.datetime(2022, 1, 1, 0, 0, 0)
    ...
    for crrt_instrument in list_instruments_to_use:
        ...
        for crrt_entry, crrt_next_entry in zip(list_measurements[:-1], list_measurements[1:]):
            ...
            if isinstance(crrt_next_entry, GNSS_Packet_hex) or isinstance(crrt_next_entry, GNSS_Packet_bin):
                position_is_valid = \
            ...
            if isinstance(crrt_next_entry, Waves_Packet):
                datetime_is_valid = crrt_next_entry.datetime_fix < date_end_ice_spectra
    ...
    for crrt_instrument in list_instruments_to_use:
        ...
        for crrt_observation, crrt_data_entry in enumerate(crrt_ordered_data):
            ...
            if isinstance(crrt_data_entry, GNSS_Packet_bin) or isinstance(crrt_data_entry, GNSS_Packet_hex):  # is it a GNSS entry?
                ...
                for crrt_ind_spectrum, crrt_energy_entry in enumerate(crrt_data_entry.list_elevation_normalized_energies):
                ...
                tz_var[crrt_trajectory, crrt_observation] = crrt_data_entry.tp
            else:
