...
with open("../generate_dict_data/dict_valid_results.pkl", "br") as fh:
    dict_observations = pickle.load(fh)

# a few params for dumping, some of them hard coded values for simplicity
number_of_valid_measurements = 6

# ------------------------------------------------------------------------------------------
print("***** prepare the dataset")

output_file = "data_boat_wave_ultrasound_probes_Barents_2021.nc"
...
with nc4.Dataset(output_file, "w", format="NETCDF4") as nc4_out:
    ...
    nc4_out.title = "Ultrasound altimeter-based measurements of waves from the Nansen Legacy Cruise PC-2: Winter Process Cruise (WPC), 2021"

    nc4_out.summary = \
        "The wave measurements using ship-based ultrasound anemometer and IMU from the Nansen Legacy Cruise PC-2: Winter Process Cruise 2021. The " +\
        ...
        "https://github.com/jerabaul29/data_release_sea_ice_drift_waves_in_ice_marginal_ice_zone_2022/tree/master/Data/2021_February_Barents/UG_Probe . " +\
    ...
    nc4_out.date_created = "2021-09-30:00:00:Z"
    ...
    nc4_out.references = "XX: report about WPC 2021: not published yet "
    ...
    for crrt_key in dict_observations:
        for crrt_position in dict_observations[crrt_key]["boat_positions"]:
            crrt_lat = crrt_position.latitude
            crrt_lon = crrt_position.longitude
            crrt_datetime = crrt_position.datetime_fix
    ...
    for crrt_segment_ind, crrt_segment_key in enumerate(dict_observations):
        time_start_segment_var[crrt_segment_ind] = crrt_segment_key[0].timestamp()
        time_end_segment_var[crrt_segment_ind] = crrt_segment_key[1].timestamp()

        lat_start_segment_var[crrt_segment_ind] = dict_observations[crrt_segment_key]["boat_positions"][0].latitude
        lat_end_segment_var[crrt_segment_ind] = dict_observations[crrt_segment_key]["boat_positions"][-1].latitude

        lon_start_segment_var[crrt_segment_ind] = dict_observations[crrt_segment_key]["boat_positions"][0].longitude
        lon_end_segment_var[crrt_segment_ind] = dict_observations[crrt_segment_key]["boat_positions"][-1].longitude
        ...
        tz_var[crrt_segment_ind] = dict_observations[crrt_segment_key]["Tz"]
        tm_var[crrt_segment_ind] = dict_observations[crrt_segment_key]["Tp"]
