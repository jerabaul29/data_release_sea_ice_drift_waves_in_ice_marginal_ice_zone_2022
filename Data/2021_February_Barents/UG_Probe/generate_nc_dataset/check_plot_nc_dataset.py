...
input_file = "./data_boat_wave_ultrasound_probes_Barents_2021.nc"
...
with nc4.Dataset(input_file, "r", format="NETCDF4") as nc4_fh:
    ...
    for crrt_segment in range(number_of_segments):
        ...
        time_start_segment = datetime.fromtimestamp(float(nc4_fh["time_start_segment"][crrt_segment].data))
        swh = float(nc4_fh["swh"][crrt_segment].data)
        hs = float(nc4_fh["hs"][crrt_segment].data)
        tp = float(nc4_fh["tp"][crrt_segment].data)
        tm = float(nc4_fh["tm"][crrt_segment].data)
