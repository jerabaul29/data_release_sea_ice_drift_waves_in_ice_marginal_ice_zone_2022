...
with open("./dict_data_each_logger_status.pkl", "rb") as fh:
...
ax.set_extent([13, 34, 75.0, 79.5])

# plot the trajectories
for ind, crrt_instrument in enumerate(dict_data_each_logger.keys()):
    # get the data
    list_datetime = [crrt_fix.datetime for crrt_fix in dict_data_each_logger[crrt_instrument]["GPS"]]
    list_latitude = [crrt_fix.latitude for crrt_fix in dict_data_each_logger[crrt_instrument]["GPS"]]
    list_longitude = [crrt_fix.longitude for crrt_fix in dict_data_each_logger[crrt_instrument]["GPS"]]

    # get instrument ID
    ID = crrt_instrument[11:]
    while len(ID) < 5:
        ID = "0" + ID
    ...
    plt.plot(list_longitude, list_latitude, transform=ccrs.PlateCarree(), zorder=3, linewidth=4.0, label=ID, color=list(mcolors.TABLEAU_COLORS)[ind])
...
plt.savefig("all_trajectories_AeN2021_instrumentsv2018.png")
plt.savefig("all_trajectories_AeN2021_instrumentsv2018.pdf")
