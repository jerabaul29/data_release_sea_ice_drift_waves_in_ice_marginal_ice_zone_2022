...
with open("../generate_dict_data/data_from_drifter_Aleksey.pkl", "br") as fh:
    all_data = pickle.load(fh, encoding='latin1')

nbr_of_instruments = 1
max_nbr_of_samples = len(all_data)
drifter_name = "drifter_0000"

# ------------------------------------------------------------------------------------------
print("***** prepare the dataset")

output_file = "data_drift_2018_Spitsbergen_Bank.nc"
...
with nc4.Dataset(output_file, "w", format="NETCDF4") as nc4_out:
    ...
    nc4_out.title = "Sea ice and open water drift from the 2018 deployment on the Spitsberg banks."

    nc4_out.summary = \
        "The sea ice drifter trajectories from the 2018 Spitsbergen banks deployment by Marchenko et. al., generated with the code at: " +\
        "https://github.com/jerabaul29/data_release_sea_ice_drift_waves_in_ice_marginal_ice_zone_2022/tree/master/Data/2018_Spitsbergen_Bank . " +\
    ...
    nc4_out.creator_name = "data from Aleksey Marchenko, packaged by Jean Rabault"
    ...
    nc4_out.project = "Centre for Sustainable Arctic Marine and Coastal Technology (SAMCoT) and IntPart project Arctic " +\
        "Offshore and Coastal Engineering in Changing Climate (AOCEC)."

    nc4_out.instrument = "GNSS"

    nc4_out.references = "Marchenko, A., et al. 'Properties of drift ice and surface currents on Spitsbergen Bank.' "+\
        "Proceedings of the 26th International Conference on Port and Ocean Engineering under Arctic Conditions (POAC), June 14–18, Moscow, Russia. 2021."
    ...
    for crrt_instrument in [drifter_name]:
        ic(crrt_instrument)
        crrt_dict_instr = all_data
    ...
    for crrt_instrument in [drifter_name]:
        ...
        crrt_trajectory = [drifter_name].index(crrt_instrument)  # the trajectory number; so that numbering fits with the index of the instrument to use
