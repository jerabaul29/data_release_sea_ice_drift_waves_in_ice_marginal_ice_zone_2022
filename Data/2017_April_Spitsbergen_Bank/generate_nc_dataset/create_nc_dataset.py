...
with open("../generate_dict_data/data_from_drifter_Ian.pkl", "br") as fh:
...
output_file = "data_drift_2017_April_Spitsbergen_Bank.nc"
...
with nc4.Dataset(output_file, "w", format="NETCDF4") as nc4_out:
    ...
    nc4_out.title = "Sea ice drift from the April 2017 deployment on the Spitsberg banks."
    ...
    nc4_out.creator_name = "data from Ian Turnbull, packaged by Jean Rabault"
    ...
    nc4_out.project = "Centre for Sustainable Arctic Marine and Coastal Technology (SAMCoT), " +\
        "the Arctic Offshore and Coastal Engineering in a Changing Climate (AOCEC) project of the Programme for International Partnerships for Excellent Education, Research, and Innovation (IntPart), " +\
        "the Dynamics of Floating Ice (DOFI) project of the Large-scale Programme for Petroleum Research (Petromaks2). "

    nc4_out.instrument = "GNSS"

    nc4_out.references = "Deformation of an Ice Pack Influenced by Waves and Topographic Enhancement of Tidal Motion near Hopen Island in the Barents Sea, "+\
        "Cold Regions Science and Technology, vol. 194, 103463, doi: https://doi.org/10.1016/j.coldregions.2021.103463."
    ...
    lon_min = 180.0
    lon_max = -180.0
    datetime_start = datetime.datetime(2030, 1, 1, 0, 0, 0)
    datetime_end = datetime.datetime(2000, 1, 1, 0, 0, 0)
