...
input_file = "./data_drift_2017_April_Spitsbergen_Bank.nc"
...
for crrt_index, crrt_instrument_id in enumerate(list_keys_ordered_by_first_lat):
    ...
    crrt_list_times = sorted(list(crrt_dict_time_pos.keys()))
    # to find when the tracker went off the ice; set to True to only get the data about the period in the ice
    # set to False to get the whole timeseries
    if True:
        print("only plot over the time when the drifter was in the ice")
        index_start = 39
        index_stop = 850
        crrt_list_times = crrt_list_times[index_start:index_stop]
        ic(crrt_list_times[0])
        ic(crrt_list_times[-1])
    else:
        print("plot the whole time series; for most of the time, the drifter is not in the ice")
