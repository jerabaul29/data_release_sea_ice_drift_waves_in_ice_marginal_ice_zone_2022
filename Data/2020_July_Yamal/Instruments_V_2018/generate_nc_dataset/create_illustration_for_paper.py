...
input_file = "./data_drift_waves_Yamal_2020_07.nc"
...
time_start = datetime(2020, 9, 6, 0, 0, 0)
time_end = datetime(2020, 9, 16, 23, 59, 0)
list_keys_ordered_by_first_lat = list_keys_ordered_by_first_lat[1:4]

# ------------------------------------------------------------------------------------------
# plot the positions on a 2D plot

print("** plot positions on simple 2D plot")

with nc4.Dataset("./multisensorSeaIce_202009161200.nc", "r", format="NETCDF4") as nc4_fh:
    sic = nc4_fh.variables['conc'][0,::1,::1]
    lon_sic = nc4_fh.variables['lon'][::1, ::1]
    lat_sic = nc4_fh.variables['lat'][::1, ::1]
    ic(sic.shape)
    ic(lon_sic.shape)
    ic(lat_sic.shape)
...
ax1.coastlines('50m')
ax1.set_extent([-3, 3, 79.5, 82], ccrs.PlateCarree())  # close lookup
#ax1.set_extent([-15, 15, 76, 85], ccrs.PlateCarree())  # high level

sic_filtered = sic
pc = plt.pcolormesh(lon_sic, lat_sic, sic_filtered, alpha=0.5,linewidth=0, vmin=0, vmax=100, antialiased=True)
#plt.axis('off')
...
for crrt_index, crrt_instrument_id in enumerate(list_keys_ordered_by_first_lat):
    ...
    ax1.plot(crrt_list_lon, crrt_list_lat, marker="*", linestyle="-.", alpha=0.2, color=list_colors[crrt_index])
    crrt_ind_list = []
    for crrt_ind in range(len(crrt_list_times)):
        if crrt_list_times[crrt_ind] > time_start and crrt_list_times[crrt_ind] < time_end:
            crrt_ind_list.append(crrt_ind)
    crrt_list_lat = [crrt_list_lat[i] for i in crrt_ind_list]
    crrt_list_lon = [crrt_list_lon[i] for i in crrt_ind_list]
    ax1.plot(crrt_list_lon, crrt_list_lat, label="{}".format(crrt_instrument_id), marker="*", color=list_colors[crrt_index])
    ax1.plot(crrt_list_lon[-1], crrt_list_lat[-1], marker="o", color="black")
...
for crrt_index, crrt_key in enumerate(list_keys_ordered_by_first_lat):
    ...
    if crrt_index == len(list_keys_ordered_by_first_lat)-1:
        ax1.plot(crrt_list_datetimes, crrt_list_Hs, color=crrt_color, linestyle="-", marker="*", label="{}".format(crrt_key))
    ...
    if crrt_index == len(list_keys_ordered_by_first_lat)-1:
        ...
        ax2.axis(xmin=time_start_md, xmax=time_end_md)
