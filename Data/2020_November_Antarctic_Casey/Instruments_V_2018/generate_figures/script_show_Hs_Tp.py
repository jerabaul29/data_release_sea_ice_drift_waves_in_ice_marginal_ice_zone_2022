...
from params import time_start, time_end, list_instruments
...
# %% a few parameters
REMOVE_NOISE = True
MAX_DURATION_BETWEEN_DATA = timedelta(hours=12)
...
print("available instruments: {}".format(list(dict_data_each_logger.keys())))
if list_instruments is None:
    list_instruments = list(dict_data_each_logger.keys())

print("using instruments for Hs and Tp plots: {}".format(list_instruments))
...
if True:
    ...
    for crrt_index, crrt_logger_ID in enumerate(list_instruments):
        ...
        plt.plot(crrt_datetime, crrt_SWH, color=crrt_color, linestyle='-', marker="*", label=str(crrt_logger_ID)[10:] + " H$_{S0}$")

        # using the Hs from time series
        crrt_Hs = dict_data_each_logger[crrt_logger_ID]["Hs"]
        plt.plot(crrt_datetime, crrt_Hs, color=crrt_color, linestyle='--', marker="+", label=str(crrt_logger_ID)[10:] + " H$_{St}$ ")
    ...
    for crrt_index, crrt_logger_ID in enumerate(list_instruments):
        ...
        plt.plot(crrt_datetime, crrt_T_z0, label=str(crrt_logger_ID)[10:] + " $T_{z0}$", color=crrt_color, linestyle="-", marker="*")

        # wave period from Tz
        crrt_T_z = dict_data_each_logger[crrt_logger_ID]["T_z"]
        plt.plot(crrt_datetime, crrt_T_z, label=str(crrt_logger_ID)[10:] + " $T_z$", color=crrt_color, linestyle="--", marker='+')
...
if True:
    ...
    for crrt_index, crrt_logger_ID in enumerate(list_instruments):
        ...
        pclr = plt.pcolor(crrt_datetime[1:], crrt_freq[0], np.transpose(crrt_spectra_logged)[:, 1:], vmin=-6.0, vmax=-2.0)  # we ignore the first spectrum: corrupted by deployment
