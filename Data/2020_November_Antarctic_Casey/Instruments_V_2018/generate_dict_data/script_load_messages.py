...
for crrt_rock7_id in list_rock7_ids:
    ...
    for crrt_entry in dict_entries:
        ...
        if len(crrt_date_string) == 14:
            pass
        elif len(crrt_date_string) == 13:
            crrt_date_string = crrt_date_string[0:9] + "0" + crrt_date_string[9:13]
        else:
            raise RuntimeError("invalid crrt_datetime string length")
        crrt_datetime = datetime.strptime(crrt_date_string, '%d-%m-%y %H:%M')
